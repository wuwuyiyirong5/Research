...
SchurComplement::SchurComplement(const SparseMatrix<double> &_BTx,
		 const SparseMatrix<double> &_BTy,
		 const SparseMatrix<double> &_Bx,
		 const SparseMatrix<double> &_By,
         const InverseMatrix &_Ax,
         const InverseMatrix &_Ay)
                :
                BTx(&_BTx),
                BTy(&_BTy),
                Bx(&_Bx),
                ...
                tmp22(BTy->m())
		{
		    std::cout << "A01.m() * 2 = " << BTx->m() * 2 << std::endl;
		};
