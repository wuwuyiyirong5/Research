...
void ISOP2P1::solveStokes()
{
    ...
    /// 矩阵求解. 
    dealii::SolverControl solver_control (4000, l_tol, check);
...
//    minres.solve (matrix, x, rhs, PreconditionIdentity());
	...
	FEMFunction<double, DIM> real_u(fem_space_v);
	FEMFunction<double, DIM> real_v(fem_space_v);
	...
	for (; the_element_v != end_element_v; ++the_element_v) 
	{
		...
		double dx_value = vx_value[l] - real_Vx.value(q_point[l]);
		double dy_value = vy_value[l] - real_Vy.value(q_point[l]);
		L2_err += Jxw * (dx_value * dx_value + dy_value * dy_value);
		std::vector<double> real_vx_gradient = real_Vx.gradient(q_point[l]);
		std::vector<double> real_vy_gradient = real_Vy.gradient(q_point[l]);
	...
	error = Functional::H1SemiError(v_h[0], real_Vx, 1);
...
void ISOP2P1::solveNS(int method)
{
	     ...
	     the_element_v != end_element_v; ++the_element_v) 
	{
		...
		= the_element_v->local_to_global_jacobian(quad_info.quadraturePoint());
	    int n_quadrature_point = quad_info.n_quadraturePoint();
	    std::vector<AFEPack::Point<2> > q_point 
		= the_element_v->local_to_global(quad_info.quadraturePoint());
	    /// 速度单元信息.
