...
private:
	...
	std::vector<AFEPack::Point<DIM> > logical_node_mesh_v; /**< 逻辑网格节点座标. */
	std::vector<AFEPack::Point<DIM> > displacement4mesh_p; /**< 记录压力移动位移, 为了更新速度网格. */
	std::vector<AFEPack::Point<DIM> > move_direction4mesh_v; /**< 速度网格移动方向. */
	double move_step_length_v; /**< 速度网格移动步长. */
